...
export class EmailVerificationService {
  ...
  private static async checkDomainStatus(domain: string): Promise<{ type: "allowlist" | "disposable" } | null> {
    const [domainStatus] = await db
      .select({ type: domainListsTable.type })
  ...
  static async verifyEmail(email: string, ip: string): Promise<VerificationResult> {
    ...
    // Check domain status (allowlist/blocklist) in a single query
    const domainStatus = await this.checkDomainStatus(domain);

    if (domainStatus) {
      if (domainStatus.type === "allowlist") {
    ...
    // Check similarity only if domain is not in either list
